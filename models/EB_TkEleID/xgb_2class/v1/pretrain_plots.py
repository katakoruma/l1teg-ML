...
from custom.plot.features import profile, plot_input_features, correlation_plot
from params_pu200 import features, auxiliary, samples, tag, P0, path, dir, feat_info, saturate, feat_info_normalized
import pandas as pd
import numpy as np
...
plot_input_features(
    ...
    weight="TkEle_weight",
    features=features,
    save=f"{path}/results/{dir}/plots/input_features_reweight",
)
plot_input_features(
    sig_test,
    bkg_test,
 ...
 #   weight="TkEle_weight",
    features=features,
    save=f"{path}/results/{dir}/plots/input_features",
)
...
sig_train_scale = scaler.apply(sig_train)
bkg_train_scale = scaler.apply(bkg_train)
...
plot_input_features(
    ...
    weight="TkEle_weight",
    features=features,
    save=f"{path}/results/{dir}/plots/input_features_rescaled",
)
# %%
from matplotlib.colors import LogNorm


df_sig, df_bkg = concatenate(
    {"signal": [sig_train, sig_test], "background": [bkg_train, bkg_test]}
)

plt.hist2d(
        df_sig["TkEle_CryClu_hoe"], df_sig["TkEle_CryClu_emf"],
        bins=30, cmap='viridis', range=[[0, 1], [0.5, 1.1]], norm=LogNorm()
    )

plt.colorbar(label='Counts')
plt.title('Signal: TkEle_CryClu_hoe vs TkEle_CryClu_emf')
plt.xlabel('TkEle_CryClu_hoe')
plt.ylabel('TkEle_CryClu_emf')
plt.show()

plt.hist2d(
        df_bkg["TkEle_CryClu_hoe"], df_bkg["TkEle_CryClu_emf"],
        bins=40, cmap='viridis', range=[[-0.5, 10.], [-0.1, 1.1]], norm=LogNorm()
    )

plt.colorbar(label='Counts')
plt.title('Background: TkEle_CryClu_hoe vs TkEle_CryClu_emf')
plt.xlabel('TkEle_CryClu_hoe')
plt.ylabel('TkEle_CryClu_emf')
plt.show()
